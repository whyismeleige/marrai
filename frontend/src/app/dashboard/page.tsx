...
import ReauditButton from "@/components/dashboard/reaudit-button";
...
function avgScore(pages: BrandPage[]): number {
  ...
  return Math.round(
    scored.reduce((acc, p) => acc + (p.score ?? 0), 0) / scored.length,
  );
}
...
function avgCatScores(pages: BrandPage[]): CategoryScores {
  ...
  if (!scored.length)
    return { schema: 0, content: 0, technical: 0, structure: 0 };
...
function getSparklineData(pages: BrandPage[]): { score: number }[] {
  const all = pages
    ...
    .sort(
      (a, b) =>
        new Date(a.auditedAt).getTime() - new Date(b.auditedAt).getTime(),
    );
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-3 sm:mb-4">
        ...
        <div className="surface-card rounded-2xl p-5 sm:p-6 flex flex-col justify-between">
          ...
          <ReauditButton pageCount={pages.length} />
        </div>
      </div>
